...
private final case class D5Move(qty: Int, from: Int, to: Int)
private case object D5Move {
  def fromString(s: String): Option[D5Move] = {
    val movePattern = raw"move (\d+) from (\d+) to (\d+)".r
    s match {
      case movePattern(qty, from, to) =>
        Some(D5Move(qty.toInt, from.toInt, to.toInt))
      case _ => None
    }
  }
}
...
private case object InputParser {
  def getMoveList(i: List[String]): List[D5Move] =
    i.filter(_.startsWith("move")).flatMap(D5Move.fromString)
...
private case object Gamestate {
  ...
  def moveOneAtATime(g: Gamestate, m: D5Move): Gamestate = {
  ...
  def moveAllTogether(g: Gamestate, m: D5Move): Gamestate = {
    val srcStack = g(m.from - 1)
    val destStack = g(m.to - 1)
...
case object Day5 {
  ...
  private def solveGeneric(
      i: List[String]
  )(f: (Gamestate, D5Move) => Gamestate): String = {
  ...
  private def getMovesAndGamestate(i: List[String]): (List[D5Move], Gamestate) =
